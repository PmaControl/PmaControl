...
WHERE b.active = 1 and b.parent_id is not null and a.id='" . $id_menu . "' GROUP BY b.id "
        ...
        if (!is_string($currentLanguage) || $currentLanguage === '') {
            $currentLanguage = self::getStringValue($_GET, 'lg');
        }

        $data['language_menu'] = LanguageMenuBuilder::build(
            self::getStringValue($_GET, 'glial_path'),
            self::getStringValue($_SERVER, 'QUERY_STRING'),
            $currentLanguage,
            LANGUAGE_AVAILABLE,
            WWW_ROOT,
            I18n::$languagesUTF8
...
/**
 * Return a string value from request-like arrays without casting arrays to "Array".
 *
 * @param array<string,mixed> $source Source array.
 * @param string $key Key to read.
 * @return string String value or an empty fallback.
 */
    private static function getStringValue(array $source, string $key): string
    {
