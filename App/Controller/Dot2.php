...
class Dot2 extends Controller
{
     ...
     */
    public $maping_master           = array();
    public $master_slave            = array();
    public $galera_cluster          = array();
    //var $mysql_server            = array();
    public $servers                 = array();
    public $slaves                  = array();
    public $groups                  = array();
    public $graph_node              = array();
    ...
    public function getMasterSlave($param)
    {
        ...
        $this->slaves = Extraction::display(array("slave::master_host", "slave::master_port", "slave::seconds_behind_master", "slave::slave_io_running",
                ...
                "slave::last_sql_error", "slave::last_sql_errno", "slave::using_gtid", "variables::is_proxysql"), "ALL");
    ...
    public function pushUpdateMS()
    {
        foreach ($this->slaves as $connections) {
            foreach ($connections as $connection_name => $slave) {
                ...
                } elseif ($slave['seconds_behind_master'] !== "0" && $slave['slave_io_running'] === "Yes" && $slave['slave_sql_running'] === "Yes") {
        ...
        if (!empty($databases)) {
            ...
            foreach ($databases as $database) {
                ...
                if ($database['binlog_do_db'] == "1") {
                    ...
                    $node .= "&#10006;";
                } elseif (!empty($database['binlog_ignore_db']) && $database['binlog_ignore_db'] == "1") {
                    $node .= "&#10004;";
                } else {
                    $node .= "-";
                }
        ...
        foreach ($this->graph_edge as $id_slave => $masters) {
            foreach ($masters as $id_master => $val) {
                ...
                if (in_array($id_slave, $group)) {
                    ...
                    $server_available = Common::getAvailable();

                    $server_ok = explode(',', $server_available['available']);

                    // si le serveur est HS on surcharge la replication
                    if (!in_array($id_slave, $server_ok)) {
