...
$(function() {
    $(document).on("click", "input.server", function(e) {
        $(this).autocomplete(GLIAL_LINK+"backup/getServerByName/ajax>true/", {
            mustMatch: true,
            autoFill: true,
            max: 100,
            minChars: 2,
            ...
            if (data)
            {
                ...
                $(this).parent().next().next().find("select").load(GLIAL_LINK+"backup/getDatabaseByServer/" + data[1] + "/ajax>true/");
            }
        }
        );
    });
...
$(function() {
    $(document).on("click", "input.ip", function(e) {
        $(this).autocomplete(GLIAL_LINK+"backup/getServerByIp/ajax>true/", {
            mustMatch: true,
            autoFill: true,
            max: 100,
            minChars: 2,
            ...
            if (data)
            {
                ...
                $(this).closest('tr').find("input.auto").val(data[1]);
                $(this).parent().next().find("select").load(GLIAL_LINK+"backup/getDatabaseByServer/" + data[1] + "/ajax>true/");
            }
        }
        );
    });
