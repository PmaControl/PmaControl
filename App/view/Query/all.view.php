...
function format_count($n): string {
    ...
    //return number_format($n / 1_000_000_000_000_000_000, 2) . "Qi";
}
...
function format_count_decimals($n, int $precision = 2): string {
    ...
    //return number_format($n / 1_000_000_000_000_000_000, $precision) . "Qi";
}
...
$date    = max(array_column($rows, 'date') ?: [1]);
...
<div class="panel-heading">
   <h3 class="panel-title"><?= __("Queries") ?> : <?= Display::srv($param[0]) ?> - 
        <span class="label <?= age_color($date); ?>">
            <?= human_time_diff_dec($date,0); ?>
        </span>&nbsp;Last refresh : <?=$date  ?>
</h3>
</div>
...
foreach ($rows as $digest_id => $sql) {

    $line = $data['queries'][$digest_id] ?? null;
    if (empty($line['digest_text'])) continue;
    $i++; if ($i > 200) break;
...
// QUERY CELL WITH FLOAT RIGHT ICONS
...
/*
    echo '<td class="col-query">
            <a href="'.LINK.'Query/digest/'.$param[0].'/'.$line['schema_name'].'/'.$line['digest'].'/" style="color:black;text-decoration:none;">'
            .highlight_mariadb_keywords($line['digest_text']).'</a></td>';*/

    echo '<td><div class="progress-wrap"><span class="progress-value">'.format_count($sql['count_star']).'</span><div class="glial-progress-bar"><div class="glial-progress" style="width:'.$p_req.'%;"></div></div></div></td>';

    echo '<td><div class="progress-wrap"><span class="progress-value">'.format_count_decimals($rq, 2).'</span><div class="glial-progress-bar"><div class="glial-progress" style="width:'.$p_rq.'%;"></div></div></div></td>';

    echo '<td><div class="progress-wrap"><span class="progress-value">'.format_latency($sql['avg_timer_wait']).'</span><div class="glial-progress-bar"><div class="glial-progress" style="width:'.$p_lat.'%;"></div></div></div></td>';

    echo '<td><div class="progress-wrap"><span class="progress-value">'.format_latency($sql['sum_timer_wait']).'</span><div class="glial-progress-bar"><div class="glial-progress" style="width:'.$p_total.'%;"></div></div></div></td>';
