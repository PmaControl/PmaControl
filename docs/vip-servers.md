...
## 6) Instance zol-galera ProxySQL

- VIP réseau : `10.68.68.250/24`.
- Port applicatif : ProxySQL `6033`.
- Noeuds keepalived : `10.68.68.231`, `10.68.68.232`, `10.68.68.233`.
- Ordre VRRP : `.231` priorité `150`, `.232` priorité `140`, `.233` priorité `130`.
- Check local : `/usr/local/sbin/check-proxysql-6033.sh` valide `127.0.0.1:6033`; si le listener ne répond pas, le noeud perd la VIP.
- Entrée PmaControl : `id_mysql_server=268`, `zol-galera-proxysql-vip-sql`, `is_vip=1`, `is_proxy=1`.
- Détection courante : le worker VIP rattache l'actif au serveur physique qui porte `10.68.68.250`; pour le service SQL, l'endpoint PmaControl reste `10.68.68.250:6033`.
