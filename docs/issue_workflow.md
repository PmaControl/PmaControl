...
2. For each issue, post an initial handling summary, create a dedicated branch from an up-to-date `origin/master`, and never modify `master` directly. New PRs must target `master`; do not use `commercial` as the target branch unless the user explicitly overrides this rule for a specific task.
