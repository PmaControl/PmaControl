...
2. For each issue, post an initial handling summary, create a dedicated branch from an up-to-date `origin/master`, and never modify `master` directly. Use `commercial-issue-XXX` as the branch name if useful as a project namespace, but PRs target `master` directly, not `commercial`. The previous two-step "PR -> commercial, then merge commercial -> master" flow is dropped because it created silent merge artefacts such as file content reverting during the second merge.
...
7. Push the branch, open the PR against `master`, link the PR in the issue, merge only after both reviews agree, post the final PR and merge commit links, close the issue, then continue with the next issue in the series.
