...
BACKUPDIR="/srv/backup/FRDC1-DR-DTA01L"
...
MAILADDR="aurelien.lequoy@68koncept.com"
...
DBEXCLUDE="performance_schema information_schema sys"
...
BACKUPFILES=""
OPT="--single-transaction --skip-add-locks --master-data=2 --quote-names --opt"			# OPT string for use with mysqldump ( see man mysqldump )
...
dbdump () {
mysqldump --port=$PORT --user=$USERNAME --password=$PASSWORD --host=$DBHOST $OPT $1 > $2
return 0
}
...
if [ "$DBNAMES" = "all" ]; then
        DBNAMES="`mysql --port=$PORT --user=$USERNAME --password=$PASSWORD --host=$DBHOST --batch --skip-column-names -e "show databases"| sed 's/ /%/g'`"
