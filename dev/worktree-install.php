...
step_webroot_config($root, $branch, $wwwPrefix, $pluginCache, $dryRun);
...
function step_webroot_config($root, $branch, $wwwPrefix, $pluginCache, $dryRun)
{
    ...
    if ($dryRun) return ok("[dry] write {$target}");
    @mkdir(dirname($target), 0755, true);
    if (file_put_contents($target, $contents) === false) {
        die_red("Cannot write {$target}");
    }
    ok("configuration/webroot.config.php → WWW_ROOT={$wwwRoot}, PLUGIN_STORAGE_DIR={$pluginCache}");
}
